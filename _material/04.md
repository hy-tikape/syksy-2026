...
FROM
  Results R
WHERE
  EXISTS (SELECT 1 FROM Results WHERE score < R.score);
```

Tässä kyselynä on `SELECT 1`, mikä tarkoittaa, että jokaisella tulosrivillä on pelkkä luku 1. Tällainen kysely on kätevä, kun riittää tietää, onko taulussa jotain ehtoon sopivaa riviä, eikä ole tarvetta saada tietoa rivin tarkemmasta sisällöstä.
...
Monimutkaisen kyselyn rakentamista voi helpottaa, jos kyselyn pystyy jakamaan useampaan itsenäiseen osaan. Tutustutaan seuraavaksi komentoihin `UNION` ja `WITH`, joita voi käyttää kyselyn osittelussa.

### Komento UNION

Komennon `UNION` avulla tulostaulu voidaan luoda yhdistämällä useamman `SELECT`-kyselyn tuottamat tulostaulut. Tästä on hyötyä, jos halutun tulostaulun pystyy saamaan aikaan yhdistämällä tuloksia eri `SELECT`-kyselyistä.
...
Nyt voimme hakea seuraavasti kaikki tauluissa esiintyvät nimet:
...
Komennon `WITH` avulla kyselyn avuksi voidaan luoda nimetty apukysely. Tällaisesta kyselystä käytetään usein nimeä _common table expression_ (_CTE_).
...
Tässä osiossa on lisäesimerkkejä SQL:n käyttämisestä. Näistä ideoista on hyötyä joidenkin SQL Trainerin vaikeiden tehtävien ratkaisemisessa.
