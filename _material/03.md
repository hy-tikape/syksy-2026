...
Tässä sarake `teacher_id` viittaa taulussa `Teachers` olevan rivin id-numeroon, minkä tuo esille viittausmerkintä `REFERENCES Teachers`.
...
Viittausmerkintä `REFERENCES` ei ole pakollinen, vaan äskeisen taulun voisi määritellä myös näin:
...
Kuitenkin viittausmerkinnän käyttäminen on hyvä tapa kahdesta syystä:

1. Se tuo esille taulun määrittelyn lukijalle, että kyseisen sarakkeen on tarkoitus viitata toisen taulun riviin.

2. Sen avulla tietokanta voi valvoa, että viittaus kohdistuu todelliseen toisessa taulussa olevaan riviin. SQLitessä viittausten valvonta täytyy ottaa käyttöön erikseen, ja palaamme asiaan luvussa 7.
...
SQLiten rajoituksena oli pitkään, että vain `JOIN` ja `LEFT JOIN` olivat mahdollisia, mutta versiosta 3.39.0 alkaen myös `RIGHT JOIN` ja `FULL JOIN` ovat käytettävissä. Tämän kurssin materiaalissa käytämme `LEFT JOIN` -syntaksia, kun tuloksiin halutaan myös rivit, jotka eivät yhdisty toisen taulun riveihin.
